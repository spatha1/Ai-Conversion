          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ py: 1.5 }}>
              ...
              <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap', fontSize: '0.78rem' }}>
                ...
                {outputXml  && (
                  <Box>
                    <strong>Generated:</strong> {outputXml.generated} &nbsp;
                    <strong>Groups:</strong> {outputXml.groups}
                  </Box>
                )}
                {run.target_url && (
                  <Box>
                    <strong>Target:</strong> {run.target_url} &nbsp;
                    {run.target_status != null && (
...
export default function ExecuteTab() {
  const { activeConnection, activeProject } = useAppStore()
  const qc = useQueryClient()
  const [targetUrl, setTargetUrl]   = useState('')
  const [lastRunId, setLastRunId]   = useState<number | null>(null)

  const { data: runs = [], isFetching, refetch } = useQuery({
    queryKey: ['runs', activeProject?.id, activeConnection?.id],
    queryFn:  () => runEngineApi.list({
  ...
  if (!activeConnection) {
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="info">
          Select a connection from the top header to run the conversion pipeline.
        </Alert>
      </Box>
    )
  }
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100 }}>
      ...
      <Paper variant="outlined" sx={{ p: 2.5, mb: 3, borderRadius: 2 }}>
        <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 0.5 }}>Trigger Run</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2, fontSize: '0.8rem' }}>
          Runs the full pipeline for <strong>{activeConnection.name}</strong> — fetches source data, fills the XML template, and stores output.
        </Typography>
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <Button
            ...
            onClick={() => { triggerMutation.reset(); triggerMutation.mutate() }}
        ...
        {triggerMutation.isSuccess && lastRunId && (
          <Alert
            severity={
              triggerMutation.data?.status === 'success' ? 'success' :
              triggerMutation.data?.status === 'failed'  ? 'error'   : 'warning'
            }
            sx={{ mt: 2, fontSize: '0.8rem' }}
          >
            Run <strong>#{lastRunId}</strong> completed — status: <strong>{triggerMutation.data?.status}</strong>.
            {triggerMutation.data?.output_xml && (() => {
              try {
                const o = JSON.parse(triggerMutation.data.output_xml!)
                return ` Generated ${o.generated} XML record(s) from ${o.groups} group(s).`
              } catch { return '' }
            })()}
          </Alert>
        )}
      ...
      <Paper variant="outlined" sx={{ borderRadius: 2 }}>
        ...
        {runs.length === 0 ? (
          <Box sx={{ p: 4, textAlign: 'center', color: 'text.secondary' }}>
            <Typography variant="body2">No runs yet for this connection. Trigger the first run above.</Typography>
          </Box>
        ) : (
          <Table size="small">
            <TableHead>
